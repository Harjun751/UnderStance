...
beforeEach(() => {
    const mockFetchCall = vi.fn()
      .mockResolvedValueOnce({
        ...
        json: () => Promise.resolve([
          { StanceID: 1, Stand: true, Reason: "TEST: it's a bop", IssueID: 1, PartyID: 1 },
        ]),
      })
      .mockResolvedValueOnce({
        ok: true,
        json: () => Promise.resolve([
          { PartyID: 1, Name: "Test Party", ShortName: "TP", Icon: "..." },
        ]),
      })
      .mockResolvedValueOnce({
        ok: true,
        json: () => Promise.resolve([
          { IssueID: 1, Description: "TEST: cool", Summary: "First Test" },
          { IssueID: 2, Description: "TEST: cool2", Summary: "Second Test" },
        ]),
      });
    ...
    vi.mock(import('react-router-dom'), async (importOriginal) => {
        ...
        return {
            ...mod,
            useLocation: vi.fn().mockResolvedValue({ state: { answers: null }})
        };
    });
});
...
describe("filters when search is input", () => {
    ...
    test('search makes other elements invisible', async () => {
        ...
        const firstIssue = listItems.find(item => item.getAttribute('issueid') === '1');
        const secondIssue = listItems.find(item => item.getAttribute('issueid') === '2');
